...
const Dashboard = () => {
  ...
  const [driver,setDriver] = useState([]);
    ...
    useEffect(() => {
        axios.get('/all-traveled')
            .then(res => setTraveled(res.data));
        axios.get('/all-user')
          .then(res => {
            setUsers(res.data);
            const dx = users.filter(user => user.role == 'Driver');
            return setDriver(dx);
          });
  ...
  return (
    <div className="p-6  min-h-screen flex flex-col items-center">
      ...
      <div className="w-full flex justify-around mb-6">
        ...
        <InfoBox title="Total Drivers" value={driver.length} />
        <InfoBox title="Total Income" value={traveled?.length*20} />
      </div>
